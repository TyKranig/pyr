...
for season in SEASONS:
    ...
    topKills = performancesColl.getData(
        "kills", 10, -1, {"seasonNumber": season[0]})
    writer.writeArray("A2:C11", topKills, "kills", "steamName", "dotabuff")

    topDeaths = performancesColl.getData(
        "deaths", 10, -1, {"seasonNumber": season[0]})
    writer.writeArray("A13:C22", topDeaths, "deaths", "steamName", "dotabuff")

    topAssists = performancesColl.getData(
        "assists", 10, -1, {"seasonNumber": season[0]})
    writer.writeArray("A24:C33", topAssists, "assists",
                      "steamName", "dotabuff")

    topGPM = performancesColl.getData(
        "gold_per_min", 10, -1, {"seasonNumber": season[0]})
    writer.writeArray("A35:C44", topGPM, "gold_per_min",
                      "steamName", "dotabuff")

    longDuration = gamesColl.getData(
        "duration", 10, -1, {"seasonNumber": season[0]})
    writer.writeArray("A46:B55", longDuration, "string_duration", "dotabuff")

    shortDuration = gamesColl.getData(
        "duration", 10, 1, {"seasonNumber": season[0]})
